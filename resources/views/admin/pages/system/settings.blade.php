    ...
    <div class="card">
        ...
        <div class="card-body pt-0">
            <ul class="nav nav-stretch nav-line-tabs nav-line-tabs-2x border-transparent fs-5 fw-bold gap-5 mb-8">
                ...
                <li class="nav-item">
                    <a class="nav-link pb-4" data-bs-toggle="tab" href="#system_settings_paytr_tab">
                        <i class="fa fa-credit-card me-2 text-success"></i>PayTR
                    </a>
                </li>
            </ul>
...
$(document).ready(function () {
    $('#paytrSaveBtn').on('click', function () {
        var btn = document.getElementById('paytrSaveBtn');
        btn.setAttribute('data-kt-indicator', 'on');
        btn.disabled = true;
        $.ajax({
            url: '{{ route("admin.paytrSave") }}',
            type: 'POST',
            data: $('#paytrSettingsForm').serialize(),
            headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'},
            success: function (res) {
                btn.removeAttribute('data-kt-indicator');
                btn.disabled = false;
